...
function startHttp() {
	...
	if(cookies.prev) { // If prev is in the cookie
	...
	if(req.url.length > 1) {
	    ...
	    if(urlObj.pathname == "/fact" && urlObj.query.hash && urlObj.query.hash.length == 32 && factLookup[urlObj.query.hash]) 
		fact = factLookup[urlObj.query.hash]; // Get that bitch
	}
	
	/* If not a directly linked fact then we randomly get one */
	if(!fact) { // If a fact hasn't been gotten then get one
	    var f = function(g) { 
...
function updateFacts() {
	...
	if(newFacts.length > 0) {
	    ...
	    if(!httpServer) // If server has been setup then try to start it
		startHttp();
	} else {
...
function updateTemplate() {
	...
	if(!httpServer) // If server hasn't been setup
	    startHttp();
    });
}
